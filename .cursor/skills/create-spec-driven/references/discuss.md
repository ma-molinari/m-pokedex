...
Specifications capture WHAT to build. Design captures the architecture. But neither captures the user's vision for ambiguous areas — layout preferences, interaction patterns, error handling style, response formats, content structure. Without this, the agent guesses. With this, the agent builds what the user actually imagined.

The output — `context.md` — feeds directly into Design and Tasks:
- **Design reads it** to know which decisions are locked vs. flexible
...
| -------------------------------- | --------------------------------------------------------------- |
| Something users **SEE** (UI)     | Layout, density, interactions, empty states, visual hierarchy   |
| Something users **CALL** (API)   | Response format, errors, auth, versioning, pagination           |
| Something users **RUN** (CLI)    | Output format, flags, modes, error handling, verbosity          |
| Something users **READ** (docs)  | Structure, tone, depth, flow, navigation                        |
| Something being **ORGANIZED**    | Grouping criteria, naming, duplicates, exceptions               |
| Something being **PROCESSED**    | Error recovery, retries, partial failures, progress reporting   |
...
Present the feature boundary (from spec.md) and the gray areas to the user. Let them choose which to discuss.
...
**Question design:**
- Options should be concrete ("Card layout with 3 columns" not "Option A")
...
- **Allowed:** "How should the errors be returned?" (clarifying ambiguity)
- **Not allowed:** "Should we also add webhook notifications?" (new capability)
...
[Ideas that came up during discussion but belong in other features/phases. Captured here so they're not lost, but explicitly out of scope for this feature]
...
- **Decisions, not vision** — "JSON envelope with `data` and `error` keys" is a decision. "Should feel clean" is not.
